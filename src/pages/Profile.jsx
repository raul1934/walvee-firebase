...
export default function Profile({ user: currentUser, openLoginModal }) {
  ...
  const targetUserEmail = profileUserEmail || (isEmailInUserId ? profileUserId : null);
  ...
  const tryGetFullUserData = async (email, id) => {
    try {
      ...
      console.warn('[Profile] Authenticated full user data fetch failed:', error.message);
    }
    ...
    queryFn: async () => {
      ...
      if (targetUserEmail) {
        ...
        try {
          ...
          if (userTripsByEmail.length > 0) {
            ...
            console.log('[Profile] Built partial user from trips:', userFromPublicData);
          }
        } catch (error) {
          console.error('[Profile] Error searching by trips for public profile:', error);
        }
      }
      ...
      if (currentUser) {
        ...
        try {
          ...
          console.warn('[Profile] Authenticated direct user access failed, potentially due to permissions or user not found:', error.message);
        }
      }
      ...
      console.error('[Profile] User not found with any method (public or authenticated).');
      return null;
    },
  ...
  const { data: followStatus } = useQuery({
    ...
    queryFn: async () => {
      if (!currentUser || !profileUser || !profileUser.id || isOwnProfile) return null; // Needs profileUser.id for follow
      
      const cached = sessionStorage.getItem(`followStatus_${profileUser.id}_${currentUser.id}`);
      if (cached) {
      ...
      try {
        ...
        sessionStorage.setItem(`followStatus_${profileUser.id}_${currentUser.id}`, JSON.stringify({
          data: result,
          timestamp: Date.now()
        }));
        ...
        console.warn('[Follow] Error checking status (might not be authenticated for follow entity):', error.message);
        return null;
      }
    },
  ...
  const { favorites: userFavorites, isLoading: isLoadingFavorites } = useFavorites(profileUser);
  ...
  const fetchFollowRecords = async ({ pageParam = 0, type, userId }) => {
    ...
    const filterOptions = type === 'followers'
      ? { followee_id: userId }
      : { follower_id: userId };
    ...
    try {
      ...
      return {
        data: orderedUsers,
        nextPage: followRecords.length === USERS_PAGE_SIZE ? pageParam + 1 : undefined,
      };
    } catch (error) {
      console.error('[Profile] Error fetching follow records (requires authentication):', error);
      return { data: [], nextPage: undefined };
    }
  };
  ...
  const {
    ...
    queryFn: ({ pageParam }) => fetchFollowRecords({ pageParam, type: 'followers', userId: profileUser?.id }),
  ...
  const {
    ...
    queryFn: ({ pageParam }) => fetchFollowRecords({ pageParam, type: 'following', userId: profileUser?.id }),
    getNextPageParam: (lastPage) => lastPage.nextPage,
  ...
  const totalPlaces = userTrips.reduce((sum, trip) => {
    return sum + (trip.itinerary?.reduce((daySum, day) => {
      return daySum + (day.places?.length || 0);
    }, 0) || 0);
  }, 0);
  
  const uniqueCountries = new Set(
    userTrips.map(trip => trip.destination?.split(',').pop()?.trim()).filter(Boolean)
  ).size;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          if (activeView === 'followers' && hasNextFollowersPage && !isFetchingNextFollowersPage) {
            fetchNextFollowersPage();
          } else if (activeView === 'following' && hasNextFollowingPage && !isFetchingNextFollowingPage) {
  ...
  const followMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const previousFollow = queryClient.getQueryData(['followStatus', profileUser?.id, currentUser?.id]);
      const previousUser = queryClient.getQueryData(['profileUser', profileUser?.id]);
      ...
      queryClient.setQueryData(['followStatus', profileUser?.id, currentUser?.id], optimisticFollow);
      sessionStorage.setItem(`followStatus_${profileUser.id}_${currentUser.id}`, JSON.stringify({
      ...
      if (previousUser) {
        const newUser = {
          ...previousUser,
          metrics_followers: Math.max(0, (previousUser.metrics_followers || 0) + 1)
        };
    ...
    onError: (err, variables, context) => {
      queryClient.setQueryData(['followStatus', profileUser?.id, currentUser?.id], context.previousFollow);
      queryClient.setQueryData(['profileUser', profileUser?.id], context.previousUser);
      alert("Couldn't follow now. Please try again.");
    },
    onSuccess: async (data) => {
      queryClient.setQueryData(['followStatus', profileUser?.id, currentUser?.id], data);
      sessionStorage.setItem(`followStatus_${profileUser.id}_${currentUser.id}`, JSON.stringify({
        data: data,
        timestamp: Date.now()
      }));

      if (profileUser.id && currentUser.id) { // Only update KPIs if IDs are available
        Promise.all([
  ...
  const unfollowMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData(['followStatus', profileUser?.id, currentUser?.id], null);
      sessionStorage.setItem(`followStatus_${profileUser.id}_${currentUser.id}`, JSON.stringify({
        data: null,
        timestamp: Date.now()
      }));
  ...
  const handleFollowClick = () => {
    ...
    if (!profileUser?.id) {
        alert("Cannot follow this user as their full profile information is not available.");
        return;
    }
      ...
      <div className="min-h-screen bg-[#0A0B0F] flex items-center justify-center pt-16">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">
            ...
            {targetUserEmail && <><br/><span className="text-xs">Searched for: {targetUserEmail}</span></>}
            {targetUserId && <><br/><span className="text-xs">ID: {targetUserId}</span></>}
          </p>
          <Button
  ...
  const firstName = profileUser.preferred_name || profileUser.full_name?.split(' ')[0] || 'Traveler';
  ...
  return (
    <div className="min-h-screen bg-[#0A0B0F] pt-16">
        ...
        <div className="bg-[#1A1B23] rounded-2xl p-5 border border-[#2A2B35]">
          ...
          <div className="flex flex-col items-center mb-4">
            ...
            <h1 className="text-lg font-bold text-white text-center mb-1">
              {profileUser.preferred_name || profileUser.full_name || 'Traveler'}
            </h1>

            <p className="text-xs text-gray-400 text-center leading-relaxed line-clamp-2 mb-3">
              {profileUser.bio || `I'm from ${profileUser.city || 'somewhere'} and a person who loves to travel and explore the natural world.`}
            </p>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <button
              ...
              className={`text-center transition-opacity ${!currentUser || followersCount === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-80'}`}
            >
              <div className="text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                {followersCount}
              </div>
            ...
            <button
              ...
              className={`text-center transition-opacity ${!currentUser || followingCount === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-80'}`}
            >
              <div className="text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                {followingCount}
              </div>
          ...
          <div className="space-y-2 mb-4 pb-4 border-b border-[#2A2B35]">
            <div className="flex items-center gap-2 text-xs text-gray-400">
              ...
              <span className="truncate">
                {profileUser.city && profileUser.country ? (
                  <>
                    {profileUser.city}, {profileUser.country.includes('-') ? profileUser.country.split('-')[1].trim() : profileUser.country}
                  </>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              ...
              <span>
                {profileUser.created_date ? format(new Date(profileUser.created_date), 'MMM yyyy') : 'Recently'}
              </span>
            </div>
          </div>
          ...
          <div className="bg-[#0D0D0D] rounded-lg p-3 mb-4">
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="text-center border-x border-[#2A2B35]">
                <div className="text-lg font-bold text-white">{totalPlaces}</div>
                <div className="text-[10px] text-gray-500">places</div>
              </div>
              
              <div className="text-center">
                <div className="text-lg font-bold text-white">{uniqueCountries}</div>
          ...
          <div className="space-y-2 mb-4">
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-[10px] ${totalTrips === 0 ? 'text-gray-600' : activeView === 'trips' ? 'text-white' : 'text-blue-400'}`}>
                  {totalTrips}
                </span>
                <ArrowRight className={`w-3 h-3 ${totalTrips === 0 ? 'text-gray-600' : activeView === 'trips' ? 'text-white' : 'text-blue-400'}`} />
              </div>
            </button>
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-[10px] ${!currentUser || userFavorites.length === 0 ? 'text-gray-600' : activeView === 'favorites' ? 'text-white' : 'text-blue-400'}`}>
                  {userFavorites.length}
                </span>
                <ArrowRight className={`w-3 h-3 ${!currentUser || userFavorites.length === 0 ? 'text-gray-600' : activeView === 'favorites' ? 'text-white' : 'text-blue-400'}`} />
              </div>
            </button>
          </div>
          ...
          {isOwnProfile ? (
            ...
            <Button
              ...
              disabled={!currentUser || followMutation.isPending || unfollowMutation.isPending || !profileUser.id}
              ...
              } ${!currentUser || !profileUser.id ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {(followMutation.isPending || unfollowMutation.isPending) ? (
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ) : (
                <>
                  {isFollowing ? `Unfollow ${firstName}` : `Follow ${firstName}`}
                </>
              )}
            </Button>
          )}
      ...
      <div className="lg:ml-[320px] xl:ml-[360px]">
        <div className="py-8 px-6">
          <main>
              ...
              <>
                ...
                {isLoadingTrips ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">
                      {isOwnProfile ? "You haven't shared any trips yet" : "This traveler hasn't shared any trips yet"}
                    </h3>
                    {isOwnProfile && (
                      <>
                        <p className="text-gray-400 mb-6 max-w-md">
                          Start creating your personalized trips with AI and share your adventures with the world.
                        </p>
                        <Button
            ...
            {(activeView === "favorites" || activeView === "followers" || activeView === "following") && !currentUser ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <div className="w-20 h-20 bg-gradient-to-br from-blue-600/20 to-purple-600/20 rounded-2xl flex items-center justify-center mb-6">
              ...
              <>
                {activeView === "favorites" && (
                  <>
                    <div className="mb-6">
                      ...
                      <p className="text-gray-400 text-sm">
                        {userFavorites.length} {userFavorites.length === 1 ? 'place' : 'places'} saved
                      </p>
                    </div>
                    ...
                    {isLoadingFavorites ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">
                          {isOwnProfile ? "No favorites yet" : `${firstName} hasn't saved any favorites yet`}
                        </h3>
                        {isOwnProfile && (
                          <p className="text-gray-400 mb-6 max-w-md">
                            Start exploring destinations and save your favorite places to find them easily later!
                          </p>
                        )}
                      </div>
                    )}
                ...
                {activeView === "followers" && (
                  <>
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-white mb-2">
                        {isOwnProfile ? 'My Followers' : `${firstName}'s Followers`}
                      </h2>
                      <p className="text-gray-400 text-sm">
                    ...
                    {(isLoadingFollowers && followers.length === 0) ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div ref={loadMoreRef} className="col-span-full">
                          {(isFetchingNextFollowersPage && hasNextFollowersPage) && (
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">
                          {isOwnProfile ? "No followers yet" : `${firstName} has no followers yet`}
                        </h3>
                        {isOwnProfile && (
                          <p className="text-gray-400 max-w-md">
                            Share your trips and connect with other travelers to grow your community!
                          </p>
                        )}
                      </div>
                    )}
                ...
                {activeView === "following" && (
                  <>
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-white mb-2">
                        {isOwnProfile ? 'Following' : `${firstName} is Following`}
                      </h2>
                      <p className="text-gray-400 text-sm">
                    ...
                    {(isLoadingFollowing && following.length === 0) ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div ref={loadMoreRef} className="col-span-full">
                          {(isFetchingNextFollowingPage && hasNextFollowingPage) && (
                            <div className="flex justify-center py-4">
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">
                          {isOwnProfile ? "Not following anyone yet" : `${firstName} isn't following anyone yet`}
                        </h3>
                        {isOwnProfile && (
                          <p className="text-gray-400 max-w-md">
                            Discover travelers and follow them to see their latest trips and recommendations!
                          </p>
                        )}
                      </div>
                    )}
      ...
      {selectedPlace && (
        <PlaceModal
          place={selectedPlace}
          trip={{ destination: `${selectedPlace.city}, ${selectedPlace.country}` }}
